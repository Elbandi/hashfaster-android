...
public class GetWorkerDataTask extends BaseDataTask {
	...
	public GetWorkerDataTask(Context context, RefreshListener listener, String pool, String key) {
	...
	protected void onPostExecute(JSONObject result) {
		if (result.length() == 0)
			return;
		try {
			MinerManager.getInstance().getMiner(mKey).setWorkers(MinerParser.parseWorkers(result));
